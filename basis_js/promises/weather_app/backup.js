...
function showWeather() {
  locationToCoordinates()
    .then(Promise.all([getCurrentWeather, getForecast]))
...
function printCurrentWeather({ main, weather, wind }) {
  ...
  rainField.innerText = `Weather today: ${weather.description}`;
  tempField.innerText = `Temperature: ${main.temp - 272.15} °C`;
  windField.innerText = `Wind: ${wind.speed} speed, ${wind.gust} gust`;
}
